...
import { Card, StatCard, StatusBadge, CategoryTag, tableStyle, thStyle, tdStyle } from './UI';
...
export default function Dashboard({ data, formulasWithCosts }) {
  const stats = useMemo(() => {
    ...
    const pendingRevenue = data.sales
    ...
    const lowStock = data.rawMaterials.filter(m => m.stock <= m.minStock).length;
    const totalProducts = data.products.reduce((s, p) => s + p.stock, 0);
    const activeClients = new Set(data.sales.map(s => s.client.name)).size;

    const salesByCategory = {};
    data.sales.forEach(sale => {
      sale.items.forEach(item => {
        const prod = data.products.find(p => p.id === item.productId);
        if (prod) salesByCategory[prod.category] = (salesByCategory[prod.category] || 0) + item.qty * item.unitPrice;
      });
    });
    ...
    data.sales.forEach(sale => {
      ...
      clientVolume[sale.client.name] = (clientVolume[sale.client.name] || 0) + total;
    });
  ...
  return (
    <div className="animate-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14, marginBottom: 22 }}>
        <StatCard icon="💰" label="Facturación Total" value={fmt(stats.totalRevenue)} color="var(--success)" sub={`${stats.totalUnits.toLocaleString()} uds vendidas`} />
        <StatCard icon="⏳" label="Por Cobrar" value={fmt(stats.pendingRevenue)} color="var(--warning)" sub={`${stats.pendingCount} pedidos`} />
        <StatCard icon="📦" label="Stock Terminado" value={stats.totalProducts.toLocaleString()} color="var(--accent)" sub="unidades en almacén" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 14 }}>
            ...
            return (
              <div key={cat.id} style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 5 }}>
                  <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}>{cat.icon} {cat.name}</span>
                  <span style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', fontSize: 12 }}>{fmt(val)}</span>
                </div>
                <div style={{ height: 6, background: 'var(--border)', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{
                    height: '100%', width: `${(val / maxCatSale) * 100}%`,
        ...
        <Card title="Clientes por Volumen">
          {Object.entries(stats.clientVolume)
            .sort(([, a], [, b]) => b - a)
            .map(([name, total], i) => (
              <div key={name} style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                ...
                <span style={{ color: 'var(--text-primary)', fontSize: 13, fontWeight: 500 }}>{name}</span>
                <span style={{
      ...
      {stats.lowStock > 0 && (
        <Card title="⚠ Materiales con Stock Bajo" style={{ marginBottom: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 10 }}>
            {data.rawMaterials.filter(m => m.stock <= m.minStock).map(m => {
              const supplier = data.suppliers.find(s => s.id === m.supplierId);
              return (
                <div key={m.id} style={{
                  background: 'var(--danger-bg)', borderRadius: 'var(--radius-md)', padding: 14,
                  border: '1px solid rgba(255,90,101,0.15)',
                }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--danger)' }}>{m.name}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4 }}>
                    {m.stock} {m.unit} disponible · Mín: {m.minStock}
                  </div>
      ...
      <Card title="Últimos Pedidos Mayoristas">
        <table style={tableStyle}>
          <thead>
            <tr>
              {['Factura', 'Fecha', 'Cliente', 'Productos', 'Total', 'Pago', 'Estado'].map(h => (
                <th key={h} style={thStyle}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {[...data.sales].reverse().slice(0, 6).map(sale => {
              const total = sale.items.reduce((s, it) => s + it.qty * it.unitPrice, 0);
              return (
                <tr key={sale.id}>
                  <td style={{ ...tdStyle, fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: 12, color: 'var(--accent)' }}>{sale.invoiceNum}</td>
                  ...
                  <td style={{ ...tdStyle, fontSize: 12 }}>
                    {sale.items.map((it, i) => {
                      ...
                      return <div key={i}>{p?.name} ×{it.qty}</div>;
                    })}
                  </td>
